...
## [0.6.3] — 2026-09-20 · Google Drive 档案介质与写包确认

### Added

- `references/google-drive.md`：按当前执行端能力选择原生 Google Docs 或 Google Drive Adapter 的路径；Adapter 以只新建的版本对象组、完成标记、回读核对和同号分叉检测实现 Google Drive 上的安全发布规则。
- E-4 的 Google Drive、发布失败、并发分叉、回读与写包前确认验收场景。

### Changed

- 倾倒在正式写包前确认来源端记忆的矛盾或关键不确定项；用户答复与原记忆状态分别保留，提交后更正改用新包。
- `config.md` 记录存储布局／格式约定和实际接入结果，但不以初始化时的能力探测锁定所有后续执行端；协议明确介质 Adapter 可采用等价物理布局。
...
- 初始化和同步都先确定并连上档案位置，再倾倒。连不上时报告原因和需要用户处理的事项后停止，不倾倒，不重新建档。
- `config.md` 在档案里，找档案位置不能从它开始：已接入的端从本端指令中的知界入口取得，新端询问用户。
- 已有档案时，先向用户说明已经接入的使用端、记忆存储和初始化状态。
- `drive.md` 中“把包先存到当前可写位置”的后备规则，只用于已取得记忆、提交时失败的情况。
- `evals.md` 新增 E-4j：新端首次使用时档案位置读写不了。

### README

- 中文 README 重写安装与使用部分：安装只保留插件方式（Claude 网页端 / Desktop、Claude Code、ChatGPT / Codex 插件目录、Codex CLI、让 AI 代装），删除技能目录复制、ZIP 上传和官方指南链接。
- “开始使用”改为第一次接入两个 AI 的三步流程；日常使用按同步、查看状态与续做、更正撤回、一致性检查组织；删除文档目录一节，改为问题反馈说明。
- 图片改用标准 Markdown 语法，新增 `docs/images/`。英文 README 尚未同步。

## [0.6.0] — 2026-09-18 · 测试版：记忆存储与使用端

测试版。以下改动只做过静态检查，尚未在任何端上端到端跑通；各端安装实测见 [#19](https://github.com/Alexu0317-FATHER/contour/issues/19)。

### 需求（`docs/新知界需求.md`）

- 区分“记忆存储”（AI 实际保存记忆的位置）和“使用端”（用户使用的产品入口）。取得信息按记忆存储计算，送回和读取按使用端计算；同一账号或同一厂商的不同端，不视为共用记忆。
- 跨端基线改为至少两个不同记忆存储的有效输入。只有一个存储时，不生成统一档案或草稿画像；能核对来源的记忆文件、导出或接口返回，可以作为该存储的输入。
- 本地、云端只描述位置，采集方式由执行者按用户授权和实际可访问的途径选择。
- 删除对 `docs/平台能力实测.md` 的依赖，各端能力以执行时的实际操作和回读结果为准。

### 技能

- 倾倒包升级为 `contour-dump-v2`：`store_id` 记内容归属，`collected_via` 记由谁读取，`source_method` 记取得方式。v1 旧包保持有效，通过 `--store-map` 换算存储身份。
- 状态文件拆为 `state/stores/`（采集与游标）和 `state/endpoints/`（加载与操作）。
- 一次同步可以采集授权范围内的多个记忆存储。已登记不等于已授权；取不到的存储记为缺口，不写成“没有变化”，也不阻塞其他存储。
- 发布只要求满足三条发布条件：同时生效、基础版本变化时不覆盖、发布后回读核对，不限定具体工具。
- 自述提示词分开标注来源类型、推断和来源不明。
- 探测和体检不再是初始化或同步的前置步骤。
- 删除 GitHub 私仓方案：网页端的 GitHub 连接器不能写入私有仓库，GitHub 当不了跨端的中间介质。协议只保留与档案位置无关的文件布局和发布条件，GH 系列检查和 `config.md` 中的 GitHub 字段一并删除。
- 新增问题反馈（`references/feedback.md`）：疑似技能缺陷时起草不含个人内容的报告，经用户同意后提交到本仓库的 issue。状态文件的操作记录增加“技能版本”一栏。
...
- `validate_dump.py`：
  - 支持 v2 和 `--store-map`；
  - 输出分为“格式检查”和“首发门槛”两段；
  - 拒绝必填字段的空值，未知写 `unknown`，没有写 `null`；
  - 单个文件的编码错误不再中断整批检查。
- `new_dump.py`：生成 v2，`--collected-via` 和 `--source-method` 改为必填。

### 分发

- 版本号从 `0.5.0-dev` 改为 `0.6.0`，同时写入 `SKILL.md` 元数据。
- 新增 Codex 插件清单 `.codex-plugin/plugin.json` 和 Codex 插件市场文件 `.agents/plugins/marketplace.json`。Codex 能否按这两个文件安装，待 #19 实测。
- 新增 issue 模板 `.github/ISSUE_TEMPLATE/bug_report.md`。
- 删除 `docs/平台能力实测.md`：其中的测试不严谨，结论已不作为能力依据。
- 按当前需求重新整理旧 issue，开放的 issue 收为 #18、#19。

## [Unreleased] — 2026-09-18 · 用户审定的记忆同步需求

- 发布用户手动审核后的 `docs/新知界需求.md`，明确跨端记忆取得、整理、送回和用户可控档案的产品目标。
- 初始化采用自然语言或显式技能入口，复用已有配置，只询问必要缺项；临时访问失败保留已有配置和初始化状态。
- 精简重复的范围与流程说明，区分部分完成、材料送达、当前会话读取和原生记忆状态，并将 MVP 验收整理为 AC-01～AC-12 场景用例。
- 本次仅更新需求文档及本条记录，不代表 Skill、README 或本地 MCP 已实现或通过验收；开发版本保持 `0.5.0-dev`。

## [0.5.0-dev] — 2026-08-14 · Codex 本地协调架构

- 重写唯一活需求文档 `docs/新知界需求.md`：Codex 成为唯一协调者和统一资产写入者，本地 Git 成为默认统一锚点，GitHub 私仓降为可选备份或跨设备迁移通道。
- 网页端不再需要直接写统一仓库：ChatGPT 通过 Codex 授权范围内的会话读取或文件包提供输入；Claude.ai 通过 Skill 生成 Markdown 包，由 Codex 的受监督浏览器流程下载、导入和送回投影。
- 本地 Agent 只写端专属交换目录并读取投影；统一资产、消费清单、端状态和 Git 历史只由 Codex 在整轮互斥事务中修改。
- 将“统一版本已提交、投影已生成、投影已送达、当前上下文已加载、原生记忆已吸收”等状态拆开记录，不再用一个“同步成功”掩盖部分完成或不可观察结果。
- 明确当前 MVP 依赖 Codex；无 Codex 的纯网页组合、多设备并行协调和无人监督浏览器自动化退出当前范围。
- 架构变化的前因、证据、旧 Issue 影响和迁移验收统一登记在 [GitHub Issue #17](https://github.com/Alexu0317-FATHER/contour/issues/17)。本条记录的是需求基线，不代表技能实现已经完成迁移或已经可以安装。

## [Unreleased] — 2026-08-14 · GitHub 架构最后审查快照

- 归档 `docs/安装前交互审查板.md`，保留审查基线 `0366ed7871e7cb6de7f0cf7f3d205728516cc99d` 下的完整问题地图，作为 GitHub 多端自治协调方案结束前的最后本地快照。
- 明确动态任务真相已经迁移到 GitHub Project 与 Issues；归档板中的 P / B / K 编号、GitHub-only 前提与“唯一检查入口”职责不再生效。
- 此提交只冻结旧架构，不包含 2026-08-14 起“Codex 唯一协调者 + 本地 Git 锚点 + 浏览器辅助线上端”的新需求改写。
