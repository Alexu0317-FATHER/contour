...
- **准入门槛**：只有被存到会话之外的产出物才登记。判据是可观察的存放动作，不推断意图
...
**不买常驻预算。** 目标是"有机会被想起"而非"每轮确定性在场"，所以走喂：只喂主题、位置和覆盖范围的**正面部分**，不喂否定边界——禁令形态在黑盒记忆里会被磨平，剩下一个膨胀的覆盖声明，比不喂更危险。召回率按 `probe.md` 的 P-3 实测，不足时再评估是否需要常驻。

### 已知未决

- **知识资产区一条真实记录都还没有**，准入门槛和调用线索的实际命中率未验证
- 资产卡片喂进黑盒端后的召回率未测（P-3 目前也没跑过）
